...
namespace GameSvr
{
    ...
    public class Merchant : NormNpc
    {
            ...
            try
            {
                if (!m_boCastle || !(m_Castle != null && m_Castle.m_boUnderWar))
                {
                    if (!PlayObject.m_boDeath && sData != "" && sData[0] == '@')
                    {
                        ...
                        else if (string.Compare(sLabel, M2Share.sDealYBme, StringComparison.OrdinalIgnoreCase) == 0) 
                        {
                            if (m_boYBDeal)
                            {
                                UserSelect_OpenDealOffForm(PlayObject); 
                            }
                        }
                    }
                }
        ...
        public void LoadMerchantScript()
        {
            ...
            M2Share.ScriptSystem.LoadScriptFile(this, M2Share.sMarket_Def, sC, true);
        }
