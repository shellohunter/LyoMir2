    ...
    public class AppService : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            if (M2Share.boStartReady)
            {
                ...
                _ = M2Share.GateManager.StartMessageQueue(stoppingToken);
            }
        ...
        public override Task StopAsync(CancellationToken cancellationToken)
        {
            ...
            var isMultiServer = M2Share.GetMultiServerAddrPort(0, ref sIPaddr, ref nPort); 
            if (isMultiServer)
            {
                    ...
                    Task.Run(() =>
                    {
                        ...
                        while (playerCount > 0)
                        {
                            ...
                            else
                            {
                                foreach (var playObject in M2Share.UserEngine.PlayObjects)
                                {
                                    if (playObject.m_boGhost || playObject.m_boDeath) 
                                    {
                                        playerCount--;
                                        continue;
                                    }
