...
namespace GameSvr
{
    ...
    public class MakeItemCommond : BaseCommond
    {
        ...
        public void CmdMakeItem(string[] Params, TPlayObject PlayObject)
        {
            ...
            var sItemName = Params.Length > 0 ? Params[0] : ""; 
            var nCount = Params.Length > 1 ? Convert.ToInt32(Params[1]) : 1; 
            var sParam = Params.Length > 2 ? Params[2] : ""; 
            ...
            if (PlayObject.m_btPermission < GameCommand.nPermissionMax)
            {
                ...
                if (M2Share.CastleManager.InCastleWarArea(PlayObject) != null) 
                {
            ...
            for (var i = 0; i < nCount; i++)
            {
                ...
                if (M2Share.UserEngine.CopyToUserItemFromName(sItemName, ref UserItem))
                {
                    ...
                    if (PlayObject.m_btPermission >= GameCommand.nPermissionMax)
                    {
                        UserItem.MakeIndex = M2Share.GetItemNumberEx(); 
                    }
